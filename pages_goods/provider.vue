	...
	<view class="content">
		<u-form labelPosition="top" :model="form" :rules="rules" ref="formRef" labelWidth="360">
			...
			<u-form-item label="联系方式" prop="tel" borderBottom>
				<u-input v-model="form.tel" border="none" type="number" maxlength="11" placeholder="请输入"></u-input>
			</u-form-item>
			...
			<u-form-item label="身份证号" prop="idCardNumber" borderBottom>
				<u-input v-model="form.idCardNumber" type="idcard" border="none" placeholder="请输入"></u-input>
			</u-form-item>
			...
			<u-form-item label="开户银行" prop="accountBank" borderBottom @click="showBank">
				<u-input
...
import { applyApi, applyUpdateApi, getApplyInfoApi, getBankApi1, getBankApi2 } from '@/api/goods'
const typeData = [
	{ name: '小微商户', id: 2401 },
	{ name: '个人卖家', id: 2500 },
	{ name: '个体工商户', id: 4 },
...
const typeData1 = [{ name: '对公账户', id: 74 }, { name: '对私账户', id: 75 }]
const progressMap = {
	0: '已申请',
	1: '已通过',
	2: '已拒绝'
...
export default {
	...
	computed: {
		...
		columns() {
			const id = this.form.bankAccountType.id
			if (id == 74) {
				return [this.bankList1]
			} else if (id == 75) {
				return [this.bankList2]
			} else {
				return []
			}
		}
	...
	onLoad() {
		getBankApi1().then(res => {
			this.bankList1 = res.data
		})
		getBankApi2().then(res => {
	...
	methods: {
		...
		showBank() {
			if (!this.form.bankAccountType.id) {
				return uni.$u.toast('请选择账户类型')
			} else {
				this.showP = true
			}
		},

<template>
	<view class="TUI-Create-conversation-container">
		<view class="tui-navigatorbar">
			<image class="tui-navigatorbar-back" @tap="goBack" src="./static/assets/ic_back_white.svg"></image>
		...
		<view class="tui-person-to-invite" v-if="searchUser.userID">
			<image
				...
				:src="isChoose ? './static/assets/single-choice-hover.svg' : './static/assets/single-choice-normal.svg'"
			></image>
...
export default {
	...
	methods: {
		...
		bindConfirmInvite() {
			if (this.isChoose) {
				uni.navigateTo({
					url: `./chat?conversationID=C2C${this.searchUser.userID}`
				});
			} else {
				uni.showToast({
					title: '请选择相关用户',
